...
from shredder.views.buddies import LocationView
from shredder.views.browser import BrowserView
from shredder.views.repo import RepoView
from shredder.views.vcs import VCSView
...
class Application(Gtk.Application):
    ...
    def do_startup(self, **kw):
        ...
        self.add_action(_create_action(
            'browse-files', lambda *_: self.win.views.switch('browser')
        ))
        self.add_action(_create_action(
            'handle-remotes', lambda *_: self.win.views.switch('locations')
        ))
        self.add_action(_create_action(
            'settings', lambda *_: self.win.views.switch('settings')
        ))
        self.add_action(_create_action(
            'handle-repositories', lambda *_: self.win.views.switch('repo'),
        ))
        self.add_action(_create_action(
            'show-history', lambda *_: self.win.views.switch('vcs'),
        ))
        self.add_action(_create_action(
        ...
        self.win.views.add_view(RepoView(self), 'repo')
        self.win.views.add_view(VCSView(self), 'vcs')
        self.win.views.add_view(LocationView(self), 'locations')
        self.win.views.add_view(BrowserView(self), 'browser')
