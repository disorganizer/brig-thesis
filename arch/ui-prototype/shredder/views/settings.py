...
from shredder.util import FileSizeRange, FileSizeSpinButton, MultipleChoiceButton
...
def numeric_widget(settings, key_name, *_, step=1):
    ...
    v = func(key_name)
    print(v)
    range_wdgt.set_value(v)
    return range_wdgt


def filesize_widget(settings, key_name, *_):
    val = settings.get_value(key_name)

    btn = FileSizeSpinButton()
    btn.set_bytes(val.get_uint64())
    return btn
...
class SettingsView(View):
    ...
    def __init__(self, app):
        View.__init__(
            self, app, sub_title='Tweak how files are synchronized'
        )
